...
def calculate_max(wire):
	lmax,rmax,dmax,umax=0,0,0,0
	pos=[0,0]
	for direction in wire:
		if direction[:1]=='L':
			...
			if pos[1]>umax:
				umax=pos[1]
	return [lmax,rmax,dmax,umax]

def calculate_size():
	size=[0,0,0,0]
	size1=calculate_max(wire1)
	size2=calculate_max(wire2)
	for i,val1,val2 in zip(range(4),size1,size2):
...
def go_through(wire, sign):
	pos = get_central_point()
	steps=-1
	for direction in wire:
		if direction[:1]=='L':
			for _ in range(int(direction[1:])):
				steps+=1
				mark(pos[0],pos[1],sign,steps)
				pos[0]-=1
...
def mark(x,y,sign,steps):
	if sign == 'first':
		if wire_map[y][x]=='not_visited':
			wire_map[y][x]='first'
		elif wire_map[y][x]!='first':
			wire_map[y][x]+=steps
			steps_array.append(wire_map[y][x])

	if sign == 'second':
		if wire_map[y][x]=='first':
			wire_map[y][x]=steps
			manhattan_distances.append(Manhattan_distance(get_central_point()[0],get_central_point()[1],x,y))

def get_central_point():
	return [abs(size[0]),abs(size[3])]
...
size = calculate_size()
central_point=[abs(size[0]),abs(size[3])]
wire_map=[ ['not_visited' for _ in range(size[0],size[1]+1)] for _ in range(size[2],size[3]+1)]
go_through(wire1,'first')
go_through(wire2,'second')
print('Minimal Manhattan distance: ' + str(min(manhattan_distances[1:])))
go_through(wire1,'first')
print(steps_array)
print('Fewest Combined steps is: ' + str(min(steps_array[1:])))
