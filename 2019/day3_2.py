def go_through(wire):
	...
	for command in wire:
		...
		for _ in range(quantity):
			x+=1 if direction=='R' else 0
			x-=1 if direction=='L' else 0
			y+=1 if direction=='U' else 0
			y-=1 if direction=='D' else 0
			step+=1
			pos[(x,y)]=step
...
for cross in crosses:
	steps.append(A[cross]+B[cross])
	cross=list(cross)	
	mdists.append(abs(cross[0])+abs(cross[1]))

print('Minimal Manhattan Distance: ',min(mdists))
print('Minimal steps to a cross: ',min(steps))
